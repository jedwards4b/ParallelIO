...
#define NUM_COMP_PROCS 1
    ...
    if (my_rank < TARGET_NTASKS)
    {
        ...
        for(int rearr=0; rearr<NUM_REARRANGERS_TO_TEST; rearr++)
        {

            /* Check for invalid values. */
            if (PIOc_init_async(test_comm, NUM_IO_PROCS, NULL, COMPONENT_COUNT,
                                num_procs, NULL, NULL, NULL, rearranger[rearr], NULL) != PIO_EINVAL)
                ERR(ERR_WRONG);
            if (PIOc_init_async(test_comm, NUM_IO_PROCS, NULL, COMPONENT_COUNT,
                                num_procs, NULL, NULL, NULL, TEST_VAL_42, iosysid) != PIO_EINVAL)
                ERR(ERR_WRONG);
            if (PIOc_init_async(test_comm, NUM_IO_PROCS, NULL, -1,
                                num_procs, NULL, NULL, NULL, rearranger[rearr], iosysid) != PIO_EINVAL)
                ERR(ERR_WRONG);
            if (PIOc_init_async(test_comm, NUM_IO_PROCS, NULL, COMPONENT_COUNT,
                                NULL, NULL, NULL, NULL, rearranger[rearr], iosysid) != PIO_EINVAL)
                ERR(ERR_WRONG);
            ...
            printf("%d %d %s SUCCESS!!\n", my_rank, rearranger[rearr], TEST_NAME);
        } /* next rearranger */
