...
/* pio specific options */
static int option_num_iotasks = 1; /* number of io tasks (>0 <=comm_size */
static int option_io_stride = 1; /* stride of io tasks */
...
main(int argc, char**argv)
{
    ...
    if (rank == 0)
    {
        ...
        opterr = 1;
        while ((c = getopt(argc, argv, "k:3467d:sum:c:h:e:rwxg:G:v:V:F:L:M:n:")) != -1) {
            switch(c) {
...
#endif
            case 'n': /* number of io-tasks (pio specific) */
                option_num_iotasks = strtol(optarg, NULL, 10);
                if(option_num_iotasks < 1 || option_num_iotasks > comm_size) {
                    error("invalid iotask value: %d", option_num_iotasks);
                }            default:
                break;
            case 'S': /* io task stride (pio specific) */
                option_io_stride = strtol(optarg, NULL, 10);
                if(option_io_stride < 1 || option_io_stride > comm_size/option_num_iotasks) {
                    error("invalid io stride value: %d", option_io_stride);
                }            default:
                break;
                usage();
            }
    ...
    if(rank != 0) inputfile = malloc(flen);
    MPI_Bcast(inputfile, flen, MPI_CHAR, 0, MPI_COMM_WORLD);
    
    if(rank == 0) flen = len(outputfile);
    MPI_Bcast(&flen, 1, MPI_INT, 0, MPI_COMM_WORLD);
    if(rank != 0) outputfile = malloc(flen);
    MPI_Bcast(outputfile, flen, MPI_CHAR, 0, MPI_COMM_WORLD);
    ...
    MPI_Bcast(option_copy_buffer_size, 1, my_MPI_SIZE_T, 0, MPI_COMM_WORLD);
    MPI_Bcast(option_chunk_cache_size, 1, my_MPI_SIZE_T, 0, MPI_COMM_WORLD);
    MPI_Bcast(option_chunk_cache_nelems, 1, my_MPI_SIZE_T, 0, MPI_COMM_WORLD);
    ...
    nc_def_iosystem(MPI_COMM_WORLD, num_iotasks, io_stride, 0, PIO_REARR_SUBSET, &iosysid);
